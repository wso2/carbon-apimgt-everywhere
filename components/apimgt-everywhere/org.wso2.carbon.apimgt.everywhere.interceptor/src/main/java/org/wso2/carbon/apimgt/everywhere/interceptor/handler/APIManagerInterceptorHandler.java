...
package org.wso2.carbon.apimgt.everywhere.interceptor.handler;
...
import org.wso2.carbon.apimgt.everywhere.interceptor.valve.APIFaultException;
			...
			if (apiService) {
				...
				if (request != null) {
					...
					if (APIUtil.getAPIContextCache().get(context) != null) {
						contextValueInCache = Boolean.parseBoolean(
								APIUtil.getAPIContextCache().get(context).toString());
					}
