...
package org.wso2.carbon.apimgt.everywhere.interceptor.valve;
...
import org.wso2.carbon.apimgt.everywhere.interceptor.APIManagerInterceptorOps;
import org.wso2.carbon.apimgt.everywhere.interceptor.UsageStatConfiguration;
import org.wso2.carbon.apimgt.everywhere.interceptor.utils.APIManagetInterceptorUtils;
...
//        context = stripVersionfromContext(context);
        ...
        if (contextExist) {
            ...
            try {
                ...
                if (!APIManagerInterceptorConstant.DEFAULT_API_VERSION.equals(apiVersion) || "".equals(apiVersion)) {
				...
				if(authLevel.equals(APIConstants.NO_MATCHING_AUTH_SCHEME)){
					...
					interceptorOps.doAuthenticate(context, APIConstants.DEFAULT_VERSION_PREFIX + apiVersion, accessToken, authLevel, domain);
				}
        ...
        if (forwardingRequired) {
            try {
                request.getRequestDispatcher(request.getContextPath() + "/services/customers/customerservice/customers/123").forward(request, response);
//                response.sendRedirect(request.getContextPath() + "/services/customers/customerservice/customers/123");
//                return;
